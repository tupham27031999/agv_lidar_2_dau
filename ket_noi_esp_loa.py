            ...
            while self.serial.inWaiting() > 0:
                ...
                if self.out != "":
                    if len(str(self.out).split("#")) >= 2:
                        ...
                        if str(self.out).split("#")[0] == "off_24v":
                            self.data_move = "off_24v#" + str(self.out).split("#")[1]
            ...
            if self.data_sent != "" and self.data_sent != self.data_move and time.time() - self.time_sent > 2:
                ...
                if self.time_sent != "":
                    try:
                        ...
                        data_sent = self.data_sent + "#0\r\n"
                        self.serial.write(data_sent.encode())
                        print("sent_data----------------", data_sent)
                    except:
                        self.connected = False
                        # break
                else:
...
def python_esp32():
    global sent_data,sent_data_new, connected, input_esp, check_connect_esp, gui_off_24v_thanh_cong
    py_esp = Python_Esp()
